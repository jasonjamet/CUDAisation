...
__global__ void kernel(float *x, float *y, float *z, int size) { 
	...
	\begin{itemize}
		...
		\item le nombre de threads total pour utiliser le minimum de ressources possible (ne pas lancer le kernel sur des coeurs inutilement)
	\end{itemize}		
	...
	Une première solution consiste à créer un analyseur syntaxique avec Flex et Bison, qui reécrira la fonction en entrée en fonction de son pragma.
	\begin{itemize}
	...
	Il est possible de créer des plugins pour le compilateur gcc. Une solution possible est donc de créer un plugin qui prend en compte les \#pragma cuda, et effectue la transformation lors de la compilation.
	\begin{itemize}
		\item Directement integré dans la compilation, ne nécéssite pas d'autres outils
	...
	\\où chaque paramètre est l'un des suivants:
	\begin{itemize}
		\item thread\_loop(i) : indique que la boucle à paralélliser est celle qui utilise la variable i
	...
	\\Nous avons ensuite modifié l'arbre pour qu'il prenne en compte ce pragma, mais qu'il puisse évidemment aussi prendre en compte les fonctions sans pragma.
